...
| **Final** | Reran with all fixes combined. | **100% catch rate on 7 deliberately corrupted examples, 0% false-flag rate on 11 genuinely clean examples** (baseline: 0% catch rate). See `output/evaluation_report.md` for the full per-example table. | The main contribution isn't the grounding-check prompt — it's the span-verification layer, which went through three real, evidence-driven corrections before it stopped producing false positives on completely legitimate citations. |
| **Generalization check** | Ran the pipeline against a second, unrelated dataset — `data-training-bot`'s own WFH_WFO survey CSV, not the synthetic policy docs — to check whether the design held up on a different data shape. | Catch rate held at 100%, but the baseline's false-flag rate on genuinely clean examples jumped to 65%. Root cause: both the baseline's and the agent's duplicate pre-filter matched on answer text alone — on short categorical answers ("Yes"/"No"/"24"), the same string legitimately recurs across unrelated questions. This was also the real explanation for the "14%" baseline catch rate above: that one catch was the same bug, a corrupted example's swapped-in answer coincidentally text-matching another example's answer. | Re-keyed both duplicate filters on `(question, answer)` instead of answer alone — a true duplicate is the same question answered the same way twice (e.g. via chunk overlap), not two different facts that happen to render as the same short string. WFH_WFO's false-flag rate dropped to 0%; the original dataset's baseline catch rate became a deterministic 0%, not a coincidence. |
